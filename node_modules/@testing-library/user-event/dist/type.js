...
async function typeImpl(element, text, {
  ...
  const setSelectionRange = newSelectionStart => {
    ...
    if (currentElement().selectionStart !== newSelectionStart) {
      ...
      (_currentElement$setSe = (_currentElement = currentElement()).setSelectionRange) == null ? void 0 : _currentElement$setSe.call(_currentElement, currentValue().length, currentValue().length);
    }
  };
  ...
  async function typeCharacter(char, {
    ...
    if (keyDownDefaultNotPrevented) {
      ...
      if (keyPressDefaultNotPrevented) {
        await fireInputEventIfNeeded({ ...calculateNewValue(key),
          eventOverrides
        });
      }
    }
